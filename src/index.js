...
//    commitsPerDay: commitsPerDay.split(","),
    commitsPerDay: "0, 2".split(","),
//    startDate: startDate ? parse(startDate) : addYears(new Date(), -1),
    startDate: "2017/01/01" ? parse("2017/01/01") : addYears(new Date(), -1),
    endDate: "2022/12/31" ? parse("2022/12/31") : new Date()
//  endDate: endDate ? parse(endDate) : new Date()
  ...
  (async function() {
    ...
    for (const date of commitDateList) {
      ...
      console.log('------------------data formatted', dateFormatted);
      //spinner.text = `Generating your Github activity... (${dateFormatted})\n`;

      await execAsync(`echo "${date}" > foo.txt`);
      await execAsync(`git add .`);
      await execAsync(`git config user.name Olaf119`);
      await execAsync(`git config user.email flyn.rider0x@gmail.com`);
...
function createCommitDateList({
  ...
  while (currentDate <= endDate) {
    ...
    var isAddDay = Math.floor(Math.random() * 1000) % 3;
    ...
    for (let i = 0; i < (Math.floor(Math.random() * 1000)) % 4; i++) {
